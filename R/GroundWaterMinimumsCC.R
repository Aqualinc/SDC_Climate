...
GWLocations <- c("Racecourse","Hororata","Charing Cross","Bankside")
GWColNames <- paste0(ScenarioType,GWSites)
parameterData <- read.table(file.path(DataDirectory,"GroundwaterModel",FileName),header=TRUE,sep=",",colClasses=c("character",rep("numeric",8)))
...
yearData <- aggregate(GWAllScenarios,by=as.numeric(format(index(GWAllScenarios),"%Y")),min)
...
png(file=file.path(ReportDirectory,"Climate","GWUpperSites-Histograms.png"),width=11,height=6,units="cm",res=600,family="Arial",pointsize=10)

oldpar <- par()
ScatterPlot <- layout(matrix(seq(1:8),2,4),widths=lcm(c(4,2,2,2)),heights=lcm(c(1.9,3.6)))
#browser()
par(mar=c(0,4,0,0),cex=1)
hist(yearData[,4],breaks=seq(min(yearData[,4]),max(yearData[,4]),l=21),yaxt="n",xaxt="n",ylab="Detrended",las=1,main="",ylim=c(0,17),xlim=c(248.7,250.4))
...
#axis(2,at=c(0,5.25,10.5,15.75),labels=c(0,0.05,0.1,0.15))
par(mar=c(4,4,0,0))
hist(yearData[,8],breaks=seq(min(yearData[,8]),max(yearData[,8]),l=21),yaxt="n",ylab="GCC",xlab=GWSites[1],las=1,main="",ylim=c(0,17),xlim=c(248.7,250.4))
axis(2,at=c(0,5.25,10.5,15.75),labels=c(0,0.05,0.1,0.15))
par(mar=c(0,0,0,0))
hist(yearData[,5],breaks=seq(min(yearData[,5]),max(yearData[,5]),l=21),xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(93,118))

#hist(yearData[,9],breaks=20,xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(12.6,14.3))
par(mar=c(4,0,0,0))
hist(yearData[,9],breaks=seq(min(yearData[,9]),max(yearData[,9]),l=21),yaxt="n",xlab=GWSites[2],main="",ylim=c(0,17),,xlim=c(93,118))
par(mar=c(0,0,0,0))
hist(yearData[,6],breaks=seq(min(yearData[,6]),max(yearData[,6]),l=21),xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(60,95))
#hist(yearData[,10],breaks=seq(min(yearData[,10]),max(yearData[,10]),l=21),xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(31,45))
par(mar=c(4,0,0,0))
hist(yearData[,10],breaks=seq(min(yearData[,10]),max(yearData[,10]),l=21),yaxt="n",xlab=GWSites[3],main="",ylim=c(0,17),xlim=c(60,95))
par(mar=c(0,0,0,0))
hist(yearData[,7],breaks=seq(min(yearData[,7]),max(yearData[,7]),l=21),xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(76,91))
#hist(yearData[,11],breaks=20,xaxt="n",yaxt="n",ann=FALSE,main="",ylim=c(0,17),xlim=c(43,54))
par(mar=c(4,0,0,0))
hist(yearData[,11],breaks=seq(min(yearData[,11]),max(yearData[,11]),l=21),yaxt="n",xlab=GWSites[4],main="",ylim=c(0,17),xlim=c(76,91))
par(oldpar)
